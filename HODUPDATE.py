...
def showhodupdate():
    ...
    def updatedata():
        ...
        xa=int(e1.get())
        xd=e5.get()
        sql="update hod set hname='%s' where hodid=%d"%(xd,xa)
        cur.execute(sql)
        db.commit()
        db.close()
        e6.delete(0,100)
    ...
    def close():
        t.destroy()
    g=Label(t,text='HOD UPDATE DATA SHEET',font='ELEPHANT 35 bold',fg='WHITE',bg='#BA6E8F')
    g.place(x=160,y=10)
    ...
    b3=Button(t,text='Update',command=updatedata,font='ariel 25 bold',bg='#98A77C',fg='white')
    b3.place(x=50,y=550)
